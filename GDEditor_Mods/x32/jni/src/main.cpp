...
int onMyLevels = 0;
bool onMyLevelsChanged = false;
int antiDeleteBug;
bool levelBrowserCallBack;
bool enteredMenu;
...
//	const char* server = AY_OBFUSCATE("http://game.gdpseditor.com/dashbx");
...
//const char* server_b64 = AY_OBFUSCATE("aHR0cDovL2dhbWUuZ2Rwc2VkaXRvci5jb20vZGFzaGJ4");
...
//const char* server_b64 = AY_OBFUSCATE("aHR0cDovL2dtZHBzZWRpdG9yLjdtLnBsL2RhdGFiYXNl"); // testing on old server
	...
	gdpsmanager->groups = GameManager::sharedState()->getGameVariable("100007");
	gdpsmanager->bh = GameManager::sharedState()->getGameVariable("100008");
...
void dict_hk1( cocos2d::CCDictionary* d, CCObject* obj, int key )
{
	...
	case 0x7DE:
        return dict1( d, CCString::create( "edit_eGShowBtn_001.png" ), key ); 
        break;
		
	case 0x7DD:
        return dict1( d, CCString::create( "edit_eGHideBtn_001.png" ), key ); 
        break;
		
	case 0x7E2:
        return dict1( d, CCString::create( "edit_eBTEOff_001.png" ), key ); 
        break;
		
	case 0x7E1:
        return dict1( d, CCString::create( "edit_eBTEOn_001.png" ), key ); 
        break;
...
*/
CCSpriteFrame* (*CCSpriteFrameCache_spriteFrameByNameO)(CCSpriteFrameCache*, const char*);
CCSpriteFrame* CCSpriteFrameCache_spriteFrameByNameH(CCSpriteFrameCache* self, const char* frame) {    
    if ( !strcmp( frame, "block005b_16_color_001.png" ) )
        return CCSpriteFrameCache_spriteFrameByNameO( self, "block005b_16_001.png" );

    if ( !strcmp( frame, "block005b_16_001.png" ) )
        return CCSpriteFrameCache_spriteFrameByNameO( self, "pixelb_03_01_001.png" );

    // stolen from gd-editor-leak lolol
    if ( !strcmp( frame, "pixelb_03_01_color_001.png" ) )
        return CCSpriteFrameCache_spriteFrameByNameO( self, "pixelb_03_01_001.png" );

    if ( !strcmp( frame, "pixelb_03_02_color_001.png" ) )
        return CCSpriteFrameCache_spriteFrameByNameO( self, "pixelb_03_02_001.png" );

    if ( !strcmp( frame, "pixelart_045_color_001.png" ) )
        return CCSpriteFrameCache_spriteFrameByNameO( self, "pixelart_045_001.png" );

    if ( !strcmp( frame, "pixelart_016_color_001.png" ) )
        return CCSpriteFrameCache_spriteFrameByNameO( self, "pixelart_016_001.png" );

    if ( !strcmp( frame, "pixelart_044_color_001.png" ) )
        return CCSpriteFrameCache_spriteFrameByNameO( self, "pixelart_044_001.png" );
	
	    if ( !strcmp( frame, "GJ_editHSVBtn2_001.png" ) ) {
			extern bool levelBrowserCallBack;
			levelBrowserCallBack = true;
        return CCSpriteFrameCache_spriteFrameByNameO( self, frame);
		}
	
	
		if(	!strcmp( frame, "GJ_stuffTxt_001.png")  ||
		!strcmp( frame, "GJ_twitterTxt_001.png")||
		!strcmp( frame, "GJ_youtubeTxt_001.png")||
		!strcmp( frame, "GJ_twitchTxt_001.png" ) ||
		!strcmp( frame, "GJ_freeStuffBtn_001.png" )
	)
        return CCSpriteFrameCache_spriteFrameByNameO( self, "transparent.png");
	
	
	    if( !strcmp( frame, "GJ_epicCoin2_001.png" )) {
			
		if(GameManager::sharedState()->getIntGameVariable("52342") >= 3) {
		
        return CCSpriteFrameCache_spriteFrameByNameO( self, "GJ_epicCoin3_001.png" );
		} else {
		return CCSpriteFrameCache_spriteFrameByNameO( self, frame );
		} 
		}
...
//for example in 50150206, there are 3 0's but 4 textures get created (5,15,2,6)
	...
	bool inMenu = GM->getGameVariable("172156");
	
	if(!inMenu) {
		
			auto sprite = CCSprite::createWithSpriteFrameName("accountBtn_myLevels_001.png");
	sprite->setScale(.75);
	auto btn = CCMenuItemSpriteExtra::create(sprite, sprite, self->m_pLayer, menu_selector(ProfilePage::onMyLevels));
	btn->setPosition(CCRIGHT - 90, CCBOTTOM + 40);
	auto menu = CCMenu::create();
	menu->setPosition(0, 0);
	menu->addChild(btn, 50);
...
*/
	...
	auto sprite = CCSprite::createWithSpriteFrameName("GJ_duplicateLockedBtn_001.png");
    auto btn = CCMenuItemSpriteExtra::create( sprite, sprite, ptr, menu_selector(LevelInfoLayer::tryCloneLevel));
	btn->setPosition(ccp(dir->getScreenLeft() + 30, dir->getScreenTop() - 60));
    auto menu = CCMenu::create();
    menu->setPosition({0,0});
	menu->addChild(btn);
    ptr->addChild(menu);
...
extern int levelBrowserCallBack;
levelBrowserCallBack = 1;
		extern bool inEditor;
		inEditor = false;
	...
	if(strstr(ret, "accSpider=") != NULL) {
        ...
        strcpy(s, ret);
       	strcat(s, toAdd);
					...
					extern int onMyLevels;
					extern bool onMyLevelsChanged;
		if (strstr(ret, "str") != NULL && onMyLevels == 1) {
			...
			switch(onMyLevels) {
				case 1:
				//onMyLevels = 0;
				//onMyLevelsChanged = true;
				toAdd2 = CCString::createWithFormat("&str=%i", GameLevelManager::sharedState()->userIDForAccountID(GM->getIntGameVariable("172157")))->getCString();
				strcat(s, toAdd2);
				break;
				/*
				case 2:
				...
				//onMyLevelsChanged = true;
				auto AM = GJAccountManager::sharedState();
				auto toAdd3 = CCString::createWithFormat("&str=%i", GLM->userIDForAccountID(AM->accountID_))->getCString();
				strcat(s, toAdd3);
				break;
				*/
			};
...
bool profile_hk(ProfilePage* self, int accountID, bool inMenu) {
	...
	GM->setGameVariable("172156", inMenu);
	if(GM->getIntGameVariable("172157") != accountID) {
	extern bool onMyLevelsChanged;
	onMyLevelsChanged = true;
	GM->setIntGameVariable("172157", accountID);
	}
	if(inMenu) {
...
void (*creatorLayeronMyLevels)(CCObject*);
void creatorLayeronMyLevels_hk(CCObject* a1) {
/*
	extern bool onMyLevelsChanged;
	extern int onMyLevels;
	onMyLevels = 2;
*/
	creatorLayeronMyLevels(a1);
}
...
bool lvlbrowser_hk(LevelBrowserLayer* self, GJSearchObject* a2) {
	...
	noRepeat++;
		...
		extern bool onMyLevelsChanged;
	if(onMyLevelsChanged && noRepeat == 0) {
	self->runAction(CCCallFuncO::create(self, callfuncO_selector(LevelBrowserLayer::onRefresh), self));
	onMyLevelsChanged = false;
	}

	return lvlbrowser(self, a2);
}

void (*LevelBrowserLayer_onBackO)(CCObject*);
void LevelBrowserLayer_onBackH(CCObject* sender){

	extern int onMyLevels;
	onMyLevels = 0;
	
	extern bool levelBrowserCallBack;
		if(levelBrowserCallBack) {
        		auto dir = CCDirector::sharedDirector();
        dir->pushScene( cocos2d::CCTransitionFade::create(0.5, CreatorLayer::scene(  ) ) );
		levelBrowserCallBack = false;
		} else {
	LevelBrowserLayer_onBackO(sender);
		}
}
...
void (*deleteBug)(CCObject*);
void deleteBug_hk(CCObject* a1) {
		
	extern int antiDeleteBug;
	if(antiDeleteBug < 2) {
	deleteBug(a1);
	} else {
	*(static_cast<std::string*>(nullptr)) = "amogus";
	}
}

void (*deleteBugPlay)(CCObject*);
void deleteBugPlay_hk(CCObject* a1) {
...
void (*profileLevels)(CCObject*);
void profileLevels_hk(CCObject* a1) {
...
bool (*searchLayer)(LevelSearchLayer*);
bool searchLayer_hk(LevelSearchLayer* self) {
	
	auto ret = searchLayer(self);
	auto win_size = CCDirector::sharedDirector()->getWinSize();

	/*
		auto customBtn = SearchButton::create("GJ_longBtn03_001.png", "Hall of Fame", 0.55, "GJ_sbigStar_noShadow_001.png");
		auto customBtn2 = SearchButton::create("GJ_longBtn03_001.png", "Most Disliked", 0.55, "GJ_sdislikesIcon_001.png");
		auto btn = CCMenuItemSpriteExtra::create(customBtn, customBtn, self, menu_selector(CreatorLayer::onFameLevels));
		auto btn2 = CCMenuItemSpriteExtra::create(customBtn2, customBtn2, self, menu_selector(CreatorLayer::onFameLevels));
	
	
	CCMenu *buttons = dynamic_cast<CCMenu *>(self->getChildren()->objectAtIndex(9));
               buttons->setPositionY(buttons->getPositionY() - 20);
				btn->setPosition(btn->getPositionX() - 90, btn->getPositionY() + 100);
				btn2->setPosition(btn2->getPositionX() + 90, btn2->getPositionY() + 100);
                buttons->addChild(btn, 50);
                buttons->addChild(btn2, 50);
				
				
				auto buttonsBG = cocos2d::extension::CCScale9Sprite::create("square02b_001.png");
            buttonsBG->setContentSize(cocos2d::CCSize(365.0f, 150.0f));
            buttonsBG->setPosition(
                    win_size.width/2,
                    win_size.height/2 + 26
            );
            buttonsBG->setOpacity(255);
            buttonsBG->setColor(ccc3(0,46,117));
            self->addChild(buttonsBG,-1);
*/
...
/*
; _DWORD LevelEditorLayer::updatePreviewAnim(LevelEditorLayer *__hidden this)
.text:002862B4                 EXPORT _ZN16LevelEditorLayer17updatePreviewAnimEv
.text:002862B4 _ZN16LevelEditorLayer17updatePreviewAnimEv
 InfoAlertButton::create(std::string, std::string, float)
.text:0038B824                 EXPORT _ZN15InfoAlertButton6createESsSsf
uwuwuwuwu
*/
...
void loader()
{
    ...
    HookManager::do_hook(getPointerFromSymbol(cocos2d, "_ZN17LevelBrowserLayer6onBackEPN7cocos2d8CCObjectE"), (void*)LevelBrowserLayer_onBackH, (void**)&LevelBrowserLayer_onBackO);
	//HookManager::do_hook(getPointerFromSymbol(cocos2d, "_ZN12CreatorLayer10onMyLevelsEPN7cocos2d8CCObjectE"), (void*)creatorLayeronMyLevels_hk, (void**)&creatorLayeronMyLevels);
	HookManager::do_hook(getPointerFromSymbol(cocos2d, "_ZN11ProfilePage10onMyLevelsEPN7cocos2d8CCObjectE"), (void*)profileLevels_hk, (void**)&profileLevels);
	...
	HookManager::do_hook(getPointerFromSymbol(cocos2d, "_ZN14LevelInfoLayer6onPlayEPN7cocos2d8CCObjectE"), (void*)deleteBugPlay_hk, (void**)&deleteBugPlay);
	HookManager::do_hook(getPointerFromSymbol(cocos2d, "_ZN14MapSelectLayer4initEi"), (void*)MapSelectLayer_hk, (void**)&MapSelectLayerO);
	HookManager::do_hook(getPointerFromSymbol(cocos2d, "_ZN14LevelInfoLayer8onDeleteEPN7cocos2d8CCObjectE"), (void*)deleteBug_hk, (void**)&deleteBug);
	...
	HookManager::do_hook(getPointerFromSymbol(cocos2d, "_ZN7cocos2d18CCSpriteFrameCache17spriteFrameByNameEPKc"), (void*) CCSpriteFrameCache_spriteFrameByNameH, (void **) &CCSpriteFrameCache_spriteFrameByNameO);
//	HookManager::do_hook(getPointerFromSymbol(cocos2d, "_ZN14EditLevelLayer4initEP11GJGameLevel"), (void*) setUpLevelInfo_hk, (void **) &setUpLevelInfo);
	...
	HookManager::do_hook(getPointerFromSymbol(cocos2d, "_ZN7cocos2d12CCDictionary9setObjectEPNS_8CCObjectEi"), (void*) dict_hk1, (void **) &dict1);
	HookManager::do_hook(getPointerFromSymbol(cocos2d, "_ZN9MenuLayer4initEv"), getPointer(&MenuLayerExt::init_hk), (void **) &MenuLayerExt::init_trp);
...
/*
	...
	//fix level too short
	tmp->addPatch("libcocos2dcpp.so", 0x241ADA, "00 bf"); */	tmp->addPatch("libcocos2dcpp.so", 0x241776, "00 bf"); //world */
	/*
	tmp->addPatch("libcocos2dcpp.so", 0x241B04, "00 bf 00 bf"); */ tmp->addPatch("libcocos2dcpp.so", 0x2417A0, "00 bf 00 bf"); //world */
	/*
	tmp->addPatch("libcocos2dcpp.so", 0x241B14, "00 bf 00 bf"); */ tmp->addPatch("libcocos2dcpp.so", 0x2417B0, "00 bf 00 bf");
	
	
	//tmp->addPatch("libcocos2dcpp.so", 0x7C066F, "6d 61 70 69 6d 61 67 65 73 2f 69 6d 61 67 65 5f 70 61 72 74 5f 30 30 30 31 2e 70 6e 67");
	
	/*
	//1101 trigger group bypass
	groups->addPatch("libcocos2dcpp.so", 0x38C80A, "20 7E"); //opacity
	groups->addPatch("libcocos2dcpp.so", 0x38E912, "20 7E"); //toggle
	groups->addPatch("libcocos2dcpp.so", 0x38FDEA, "20 7E"); //spawn
	groups->addPatch("libcocos2dcpp.so", 0x390EBC, "20 7E"); //pulse
	groups->addPatch("libcocos2dcpp.so", 0x3BFB52, "20 7E"); //stop
	groups->addPatch("libcocos2dcpp.so", 0x3C186A, "20 7E"); //animation
	groups->addPatch("libcocos2dcpp.so", 0x3C3FF6, "20 7E"); //collision
	groups->addPatch("libcocos2dcpp.so", 0x3C545E, "20 7E"); //count
	groups->addPatch("libcocos2dcpp.so", 0x3C5B62, "20 7E"); //pickup
	groups->addPatch("libcocos2dcpp.so", 0x3C7D12, "20 7E"); //instant count
	groups->addPatch("libcocos2dcpp.so", 0x3CC842, "20 7E"); //touch
	groups->addPatch("libcocos2dcpp.so", 0x3DD04A, "20 7E"); //random 1
	groups->addPatch("libcocos2dcpp.so", 0x3DD0EE, "20 7E"); //random 2
	groups->addPatch("libcocos2dcpp.so", 0x3DE3EE, "20 7E"); //end
	
	
	//move 
	groups->addPatch("libcocos2dcpp.so", 0x38AEA4, "42 F2 0F 73");
	groups->addPatch("libcocos2dcpp.so", 0x38AE70, "42 F2 0F 73");
	groups->addPatch("libcocos2dcpp.so", 0x38AF0E, "42 F2 0F 73");
	groups->addPatch("libcocos2dcpp.so", 0x38AEDA, "42 F2 0F 73");
	groups->addPatch("libcocos2dcpp.so", 0x38B6D4, "42 F2 0F 73");
	groups->addPatch("libcocos2dcpp.so", 0x38AF34, "42 F2 0F 73");
	
	//follow
	groups->addPatch("libcocos2dcpp.so", 0x3AB8FE, "20 7E");
	groups->addPatch("libcocos2dcpp.so", 0x3AB968, "20 7E");
	groups->addPatch("libcocos2dcpp.so", 0x3ADA06, "20 7E");
	groups->addPatch("libcocos2dcpp.so", 0x3AD9D2, "20 7E");
	groups->addPatch("libcocos2dcpp.so", 0x3ABD68, "20 7E");
	groups->addPatch("libcocos2dcpp.so", 0x3ADE52, "20 7E");
	groups->addPatch("libcocos2dcpp.so", 0x3AB98E, "20 7E");
	
	//rotate
	groups->addPatch("libcocos2dcpp.so", 0x3AFDE6, "20 7E");
	groups->addPatch("libcocos2dcpp.so", 0x3AFE50, "20 7E");
	groups->addPatch("libcocos2dcpp.so", 0x3B037A, "20 7E");
	groups->addPatch("libcocos2dcpp.so", 0x3AFE76, "20 7E");
	
	//static camera
	groups->addPatch("libcocos2dcpp.so", 0x3D9F36, "20 7E");
	groups->addPatch("libcocos2dcpp.so", 0x3D9F5C, "20 7E");
	
	//scale?
	groups->addPatch("libcocos2dcpp.so", 0x3F756A, "20 7E");
	groups->addPatch("libcocos2dcpp.so", 0x3F75D4, "20 7E");
	groups->addPatch("libcocos2dcpp.so", 0x3F7B02, "20 7E");
	groups->addPatch("libcocos2dcpp.so", 0x3F75FA, "20 7E");
	
		
	//build helper
	
	bh->addPatch("libcocos2dcpp.so", 0x2939DA, "42 F2 0F 73");
	bh->addPatch("libcocos2dcpp.so", 0x2939F6, "42 F2 0F 73");
	
	

	//create loop
	
	groups->addPatch("libcocos2dcpp.so", 0x293EBA, "20 7E");
	groups->addPatch("libcocos2dcpp.so", 0x294122, "20 7E");
	groups->addPatch("libcocos2dcpp.so", 0x2941B8, "20 7E");
	...
	tmp->addPatch("libcocos2dcpp.so", 0x2F8E5A, "04 e0"); */ tmp->addPatch("libcocos2dcpp.so", 0x2F9C4A, "04 e0"); //world
/*
	//creator layer original buttons
	tmp->addPatch("libcocos2dcpp.so", 0x2B1F92, "00 bf");
	*/
	tmp->addPatch("libcocos2dcpp.so", 0x2B2600, "00 bf"); //creator layer original buttons world
	tmp->addPatch("libcocos2dcpp.so", 0x2B28CA, "00 bf"); //creator layer vault world
	tmp->addPatch("libcocos2dcpp.so", 0x2B2962, "00 bf"); //creator layer vault world
	tmp->addPatch("libcocos2dcpp.so", 0x2B29BC, "00 bf"); //creator layer font
	
	tmp->addPatch("libcocos2dcpp.so", 0x2ED68E, "00 bf"); //garage layer
	tmp->addPatch("libcocos2dcpp.so", 0x2ED556, "00 bf"); //garage layer
	
	tmp->addPatch("libcocos2dcpp.so", 0x35B84A, "092B"); //global top bypass
	
	
	
	tmp->addPatch("libcocos2dcpp.so", 0x785F6A, "2f646174612f646174612f636f6d2e676470736564692e67656f6d6574727964617368776f726c642f");
	/*
	...
	tmp->addPatch("libcocos2dcpp.so", 0x2EACB8, "27"); */ tmp->addPatch("libcocos2dcpp.so", 0x2EB540, "27");
	/*

	// probably fix platformer kick
	tmp->addPatch("libcocos2dcpp.so", 0x247EC4, "00 BF 00 BF"); */ tmp->addPatch("libcocos2dcpp.so", 0x247CA4, "00 BF 00 BF");
	/*
	tmp->addPatch("libcocos2dcpp.so", 0x24168E, "00 BF 00 BF"); */ tmp->addPatch("libcocos2dcpp.so", 0x24132A, "00 BF 00 BF");
	/*
	//level serach layer nops
	tmp->addPatch("libcocos2dcpp.so", 0x2B6308, "00 BF"); //bg
	tmp->addPatch("libcocos2dcpp.so", 0x2B66EE, "00 BF"); //filters label
	tmp->addPatch("libcocos2dcpp.so", 0x2B622E, "00 BF"); //quick search label
	...
	tmp->addPatch("libcocos2dcpp.so", 0x397C6C, "BB F1 08 0F"); */ tmp->addPatch("libcocos2dcpp.so", 0x398AB4, "BB F1 08 0F");
/*
	...
	*/ // world
		groupIDLayerPatches->addPatch("libcocos2dcpp.so", 0x3E26C4, "00 bf 00 bf"); // Dont Fade
		groupIDLayerPatches->addPatch("libcocos2dcpp.so", 0x3E26C4, "00 bf 00 bf"); // Dont Enter
		groupIDLayerPatches->addPatch("libcocos2dcpp.so", 0x3E2768, "00 bf 00 bf"); // No effects
		groupIDLayerPatches->addPatch("libcocos2dcpp.so", 0x3E2872, "00 bf 00 bf"); // No effects idkkkk
		groupIDLayerPatches->addPatch("libcocos2dcpp.so", 0x3E2A20, "00 bf 00 bf"); // Group Parent
		groupIDLayerPatches->addPatch("libcocos2dcpp.so", 0x3E2AA2, "00 bf 00 bf"); // High Detail
		groupIDLayerPatches->addPatch("libcocos2dcpp.so", 0x3E2B20, "00 bf 00 bf"); // Untouchable
		groupIDLayerPatches->addPatch("libcocos2dcpp.so", 0x3E2B9E, "00 bf 00 bf"); // Passable
		groupIDLayerPatches->addPatch("libcocos2dcpp.so", 0x3E2C20, "00 bf 00 bf"); // Hide
		groupIDLayerPatches->addPatch("libcocos2dcpp.so", 0x3E2CA8, "00 bf 00 bf"); // NonStick
		groupIDLayerPatches->addPatch("libcocos2dcpp.so", 0x3E2D8C, "00 bf 00 bf"); // IceBlock
		groupIDLayerPatches->addPatch("libcocos2dcpp.so", 0x3E2DC8, "00 21"); // Order input bg
...
*/
	cameraRotatePopupPatches->addPatch("libcocos2dcpp.so", 0x3FAFA2, "27 EE 88 9A"); // VNMLS.F32  S18, S15, S16 -> VMUL.F32 S18, S15, S16 world
	/*
	cameraRotatePopupPatches->addPatch("libcocos2dcpp.so", 0x3FA348, "27 EE 88 7A"); //	VNMLS.F32  S14, S15, S16 -> VMUL.F32 S14, S15, S16
	*/
	cameraRotatePopupPatches->addPatch("libcocos2dcpp.so", 0x3FAC78, "27 EE 88 7A"); //	VNMLS.F32  S14, S15, S16 -> VMUL.F32 S14, S15, S16
